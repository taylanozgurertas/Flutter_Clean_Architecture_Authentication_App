...
//BURASI AUTHREMOTEDATASOURCEDAKI SIGN UP VE LOGIN FONKSIYONLARINI CAGIRACAK
...
class AuthRepositoryImpl implements AuthRepository {
  ...
  Future<Either<Failure, User>> loginWithEmailPassword({
    ...
    return _getUser(
      () async => await remoteDataSource.loginWithEmailPassword(email: email, password: password),
    );
  }
  ...
  Future<Either<Failure, User>> _getUser(
    ...
    try {
      final user = await fn();
      return right(user); //BASARILI!!
    } on sb.AuthException catch (e) {
      return left(Failure(e.message));
    } on ServerException catch (e) {
      return left(Failure(e.message)); //BASARISIZ!!
    }
  }
}
